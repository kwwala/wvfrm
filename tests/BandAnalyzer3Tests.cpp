...
bool runBandAnalyzerTests()
{
    ...
    const auto lowBands = analyzer.analyzeSegment(lowSignal.data(), static_cast<int>(lowSignal.size()), sampleRate, 0.25f);
    ...
    const auto midBands = analyzer.analyzeSegment(midSignal.data(), static_cast<int>(midSignal.size()), sampleRate, 0.25f);
    ...
    const auto highBands = analyzer.analyzeSegment(highSignal.data(), static_cast<int>(highSignal.size()), sampleRate, 0.25f);
